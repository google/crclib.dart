...
/// Script to generate `reveng.dart` and `test_reveng.dart`.
...
Future<bool> createLib(List<Record> records) async {
  final path = pathTo('lib', 'reveng.dart');
...
class ${record.name} extends ParametricCrc {
  ${record.name}()
      : super(
          ${record.width},
          0x${record.poly.toRadixString(16).padLeft(record.width ~/ 4, '0')},
          0x${record.init.toRadixString(16).padLeft(record.width ~/ 4, '0')},
          0x${record.xorout.toRadixString(16).padLeft(record.width ~/ 4, '0')},
...
Future<bool> createTest(List<Record> records) async {
  final path = pathTo('test', 'reveng_test.dart');
  final file = File(path);
  final sink = file.openWrite();
  sink.write(license);
  sink.write('''
...
''');
  records.forEach((record) {
    sink.write('''
  test('${record.name}', () {
    expect(${record.name}().convert(input), 0x${record.check.toRadixString(16)});
''');
    record.aliases.forEach((alias) {
      sink.write('''
    expect(${alias}().convert(input), 0x${record.check.toRadixString(16)});
''');
    });
    sink.write('  });\n');
  });
